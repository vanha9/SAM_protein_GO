...
def compute_ic(y_true):

    p_l = np.mean(y_true, axis=0)
    ic = -np.log2(p_l + 1e-10)  
    return ic

def compute_s(y_true, y_pred, ic):
    ...
    for t in np.arange(0, 1.01, 0.01):
        ...
        num =  np.logical_and(y_true == 1, y_pred_bin == 0).astype(float).dot(ic)
        denom =  np.logical_or(y_true == 1, y_pred_bin == 1).astype(float).dot(ic) + 1e-10
        ru = num / denom
        ru = np.mean(ru)
  
        num =  np.logical_and(y_true == 0, y_pred_bin == 1).astype(float).dot(ic)
        denom =  np.logical_or(y_true == 1, y_pred_bin == 1).astype(float).dot(ic) + 1e-10
        mi = num / denom
        mi = np.mean(mi)

        sd_list.append(np.sqrt(ru ** 2 + mi ** 2))

    return np.array(sd_list)

def smin(y_true, y_pred):

    ic = compute_ic(y_true)
    s_list = compute_s(y_true, y_pred, ic)
    smin = np.min(s_list)
